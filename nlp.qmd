...
Получается примерно такая таблица (при визуализации таблицы я убрала столбец **sentence**, чтобы выглядело опрятнее на странице). Полный вариант тиббла можно забрать [здесь](https://github.com/verona238/my-r-project/raw/refs/heads/main/data/ctsg_articles_pos.RData).
...
Создам еще один [тиббл](https://github.com/verona238/my-r-project/raw/refs/heads/main/data/result_words_tidy.RData), в котором будут храниться все токены без стоп-слов.
...
Как всегда представляю только небольшой фрагмент тиббла. Полную таблицу можно забрать по [ссылке](https://github.com/verona238/my-r-project/raw/refs/heads/main/data/tokens_tf.RData).
...
Ну и последнее, что я рассмотрю в этом проекте, это векторное представление слов. Ну чтош, приступим!
...
ctsg_articles_pos_all <- ctsg_articles_annotate_all |> 
  as_tibble()
```
...
Давайте посмотрим на полученную таблицу. Для preview я взяла 10 последних строк (через tail). Мне нравится, как получилось! Полную таблицу как всегда предлагаю забрать по [ссылке](https://github.com/verona238/my-r-project/raw/refs/heads/main/data/articles_ppmi.RData).
